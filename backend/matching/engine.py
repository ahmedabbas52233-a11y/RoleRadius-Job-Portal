...
def _skills_overlap(required: list, candidate_skills: list) -> tuple[float, list[str], list[str]]:
    """Return (score 0-1, matched skills, missing skills) -- exact (case-insensitive) overlap.
    ...
    req_norm = {s.strip().lower(): s for s in required if isinstance(s, str) and s.strip()}
    if not req_norm:
        return 0.5, [], []
    cand_norm = {s.strip().lower() for s in candidate_skills if isinstance(s, str) and s.strip()}
    matched = [orig for low, orig in req_norm.items() if low in cand_norm]
    missing = [orig for low, orig in req_norm.items() if low not in cand_norm]
    return len(matched) / len(req_norm), matched, missing
